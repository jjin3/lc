public class Solution {
    ...
    public int rob(int[] nums) {
        ...
        /* To draw a circle, we found that if the first house (idx=0) is robbed, the thief would not select the 
        last house (idx=nums.length-1). If the first house (idx=0) is NOT robbed, on the other hand, the thief is 
        free to select (idx=nums.length-1). Let's image two arrays with index (0, length-2) and (1, length-1). 
        These are two cases we would need to consider in this problem. It would not be more complicated than two cases,
        definitely NOT to think of a circulated array with n+1, n+2... 2n+1, 2n+2 etc.
        */
